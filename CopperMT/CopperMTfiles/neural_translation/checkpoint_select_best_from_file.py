...
if __name__ == "__main__":
    ...
    best_by_means_path = f"{work_dir}/{cp_dir}/{best_by_mean(scores)}"
    selected_path = f"{work_dir}/{cp_dir}/checkpoint_best.selected.pt"
    print(f"SELECTED: {best_by_means_path}")
    print(f"\tCOPYTING TO: {selected_path}")
    shutil.copyfile(best_by_means_path,
                    selected_path)
